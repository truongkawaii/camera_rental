...
const getSortOption = (field) =>
  SORT_OPTIONS.find((option) => option.field === field);
  ...
  return (
    <div className="relative md:hidden px-4 py-3 border-b border-slate-100 bg-white">
      <div className="flex items-center gap-2">
        <span className="text-[10.5px] font-bold uppercase tracking-widest text-slate-400 whitespace-nowrap">
          Sắp xếp:
        </span>
        <button
          type="button"
          ...
          className={`min-w-0 flex-1 h-[35px] px-3 flex items-center justify-between gap-2 rounded-xl border text-[12.5px] font-semibold transition-all ${
            open
              ? "bg-white border-orange-200 text-orange-500 ring-2 ring-orange-100"
        ...
        <button
          type="button"
          onClick={() =>
            setSortKey(buildSortKey(field, dir === "asc" ? "desc" : "asc"))
          }
...
const EMPTY_FORM = {
  ...
  user_id: "",
  handover_user_id: "",
  applied_day_price: null,
  used_discount_day_price: false,
  discount_day_price: null,
  discount_day_threshold_snapshot: null,
};
const EMPTY_CUSTOMER = { name: "", phone: "", email: "", address: "" };
const isNewImagePreview = (image) =>
  typeof image === "string" && image.startsWith("data:image/");
const getNewImageIndex = (previews, index) =>
  previews.slice(0, index + 1).filter(isNewImagePreview).length - 1;
...
const StatCard = ({
  ...
  >
    <div className="flex items-start justify-between mb-3">
      <div
        className="p-2.5 rounded-2xl transition-transform group-hover:scale-110 duration-300"
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-2 md:flex-nowrap md:justify-between">
        <div className="flex min-w-0 flex-wrap items-center gap-1.5 md:flex-nowrap">
          {trend !== null && trend !== undefined && (
            <div
              className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-lg text-[10px] font-bold ${trend >= 0 ? "text-green-600 bg-green-50" : "text-rose-600 bg-rose-50"}`}
            >
              {trend >= 0 ? (
                <TrendingUp size={10} />
              ) : (
          ...
          <span className="text-[9px] font-semibold leading-tight text-slate-400 md:min-w-0 md:truncate md:text-[10px] md:whitespace-nowrap">
            {trendLabel}
          </span>
        </div>
        {showMoreLink && (
...
const ActionRow = ({ rental, type = "pickup", onClick }) => {
  ...
  const isTest =
    rental.customer_name?.toLowerCase().includes("test") ||
    rental.notes?.toLowerCase().includes("test");
  ...
  return (
    ...
    >
      <span className="text-[13px] font-semibold text-slate-700 min-w-[80px] flex-shrink-0 group-hover:text-orange-500 transition-colors whitespace-nowrap">
        {time}
      </span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-[13.5px] font-semibold text-slate-800 truncate group-hover:text-orange-600 transition-colors">
            {rental.customer_name}
          </p>
          <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap">
            (
            {rental.code ||
              `#OD${String(rental.order_number || rental.id).padStart(7, "0")}`}
            )
          </span>
          {isTest && (
            <span className="text-[9px] font-semibold bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded uppercase tracking-tighter">
              TEST
            </span>
          )}
        </div>
        {Array.isArray(rental.items) && rental.items.length > 1 ? (
          <div className="mt-1 space-y-1">
            {rental.items.map((it, idx) => (
              <div
                key={idx}
                className="flex items-center gap-1.5 text-[11.5px] text-slate-700 font-medium"
              >
        ...
        {rental.handover_user_name && (
          <div className="flex items-center gap-1 text-[10.5px] text-blue-600 font-semibold mt-0.5">
            ...
            <span className="truncate">
              Giao nhận: {rental.handover_user_name}
            </span>
          </div>
        )}
        <div className="flex flex-col gap-0.5 mt-1">
          ...
          <div className="flex items-center gap-1 text-[10.5px] text-orange-500 font-semibold">
            ...
            <span className="truncate" title="Nơi nhận">
          ...
          <div className="flex items-center gap-1 text-[10.5px] text-emerald-600 font-semibold">
            ...
            <span className="truncate" title="Nơi trả">
              Trả:{" "}
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[13.5px] font-semibold text-slate-800 truncate group-hover:text-red-600 transition-colors">
            {rental.customer_name}
          </p>
          <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap">
            (
        ...
        <div className="flex flex-col gap-0.5 mt-1">
          <div className="flex items-center gap-1 text-[10.5px] text-indigo-500 font-semibold">
            <Home size={10} className="text-indigo-400 flex-shrink-0" />
            <span className="truncate" title="Máy thuộc">
              Máy: {rental.original_branch_name || "Hệ thống"}
            </span>
          </div>
          <div className="flex items-center gap-1 text-[10.5px] text-orange-500 font-semibold">
            <MapPin size={10} className="text-orange-400 flex-shrink-0" />
      ...
      <div className="flex flex-col items-end gap-1">
        ...
        <span
          className={`text-[11px] font-semibold whitespace-nowrap flex-shrink-0 uppercase tracking-tight ${isLatePickup ? "text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded" : "text-red-500"}`}
        >
      ...
      <ChevronRight
        size={14}
        className="text-red-300 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all"
      />
    </div>
  );
};
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-3.5 last:mb-0">
      <div className="flex items-center justify-between mb-3.5">
        ...
        <span
          className={`px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-tight ${status.cls}`}
        >
          {status.label}
        </span>
      </div>
      ...
      <div className="flex gap-3.5 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[14.5px] font-semibold text-slate-900 truncate leading-none mb-1">
            {r.customer_name}
          </p>
          {r.customer_phone && (
          ...
          {Array.isArray(r.items) && r.items.length > 1 ? (
                ...
                <div key={idx} className="flex items-start gap-1.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1">
                      <span className="text-[12px] font-semibold text-slate-800 truncate block leading-tight">
                        {it.name}
                      </span>
                      {it.is_primary && (
                    ...
                    <div className="flex items-center gap-1 text-[10px] text-indigo-500 font-medium">
                      ...
                      />
                      <span className="truncate">
                        {it.branch_name || r.original_branch_name || "Hệ thống"}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="mt-1">
                <span className="text-[11px] text-slate-400 truncate uppercase tracking-tighter block mb-0.5">
                  {r.equipment_code}
                </span>
                <div className="flex items-center gap-1 text-[11px] text-indigo-500 font-semibold">
                  <Home size={10} className="text-indigo-400 flex-shrink-0" />
                  <span className="truncate">
                    {r.original_branch_name || "Hệ thống"}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="border-t border-dashed border-slate-100 pt-3.5 space-y-2.5">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[12px] font-semibold text-emerald-600">
            {r.return_branch_name || r.pickup_branch_name || "—"}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[12px] font-semibold text-blue-600">
            {r.handover_user_name || "Chưa chỉ định"}
          </span>
        </div>
...
const ActionPanel = ({
  ...
  return (
    <div
      className={`bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden transition-all duration-300 ${!isOpen ? "max-h-[52px]" : "max-h-[500px]"}`}
    >
      <div
      ...
      >
        <div
          className={`flex items-center gap-2 text-[12.5px] font-semibold uppercase tracking-wider ${c.header}`}
        >
          <Icon size={16} />
        ...
        <div className="flex items-center gap-2">
          <span
            className={`text-[11px] font-semibold w-6 h-6 rounded-full flex items-center justify-center ${c.badge}`}
          >
            {count}
          </span>
          <ChevronDown
            size={16}
            className={`transition-transform duration-300 ${c.header} ${isOpen ? "rotate-180" : ""}`}
          />
        </div>
      </div>

      <div
        className={`flex-1 flex flex-col transition-all duration-300 ${isOpen ? "opacity-100" : "opacity-0 invisible"}`}
      >
        <div className="flex-1 overflow-y-auto max-h-72 custom-scrollbar">
          {items.length === 0 ? (
            <div className="py-8 text-center text-[12.5px] text-slate-400 italic font-medium">
              Không có dữ liệu
            </div>
          ) : (
...
const DashboardV1 = () => {
  ...
  const loadRentals = useCallback(
    async (p = 1, st = activeTab, q = search, sk = sortKey) => {
  ...
  useEffect(() => {
    ...
    const customer = customers.find(
      (c) => String(c.id) === String(editingItem.customer_id),
    );
    if (customer) {
  ...
  const openEditModal = async (item) => {
          ...
          ? [
              {
                ...
                discount_day_threshold_snapshot:
                  item.discount_day_threshold_snapshot,
    ...
    setFormData({
      ...
      discount_day_threshold_snapshot:
        item.discount_day_threshold_snapshot ?? null,
      code: item.code || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        ...
        if (
          editCustomerData &&
          String(editCustomerData.id) === String(currentFormData.customer_id)
        ) {
          const original =
            customers.find(
              (c) => String(c.id) === String(editCustomerData.id),
            ) || toEditableCustomer(editingItem);
        ...
        if (imagesDirty) {
          await uploadRentalImages(
            editingItem.id,
            imagePreviews,
            selectedImageFiles.map((file) => file.name),
          );
        }
  ...
  const SortHeader = ({ field, children, className = "", align = "left" }) => {
    ...
    return (
      <th
        className={`px-5 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-widest ${className}`}
      >
        <button
          type="button"
        ...
        >
          <span>{children}</span>
          {active ? (
            <span className="text-[11px] leading-none text-orange-500">
              {dir === "asc" ? "↑" : "↓"}
            </span>
          ) : (
  ...
  const statCards = [
    {
      ...
      label: "Doanh số",
      value: fmtVND(s.total_value_today),
      trend:
        s.total_value_today === 0 && s.total_value_trend === 0
          ? null
          : s.total_value_trend,
      trendAbs: Math.abs(s.total_value_trend ?? 0),
      trendLabel,
    },
    {
      icon: DollarSign,
      ...
      label: "Doanh thu",
      value: fmtVND(s.revenue_today),
      trend:
        s.revenue_today === 0 && s.revenue_trend === 0 ? null : s.revenue_trend,
      trendAbs: Math.abs(s.revenue_trend ?? 0),
      trendLabel,
    },
    {
      icon: FileText,
      ...
      trend:
        s.orders_today === 0 && s.orders_trend === 0 ? null : s.orders_trend,
      trendAbs: Math.abs(s.orders_trend ?? 0),
      trendLabel,
    },
    {
      ...
      iconBg: "rgba(34,197,94,0.10)",
      label: "Khách trả",
      value: s.returns_today ?? 0,
      trend:
        s.returns_today === 0 && s.returns_trend === 0 ? null : s.returns_trend,
      trendAbs: Math.abs(s.returns_trend ?? 0),
      trendLabel,
    },
    {
      icon: Camera,
      ...
      trend: s.active_rentals_trend,
      trendAbs: Math.abs(s.active_rentals_trend ?? 0),
      trendLabel,
    ...
    },
    {
      icon: Package,
      iconBg: "rgba(249,115,22,0.10)",
      label: "Thiết bị trống",
      value: s.available_equipment ?? 0,
      trend: s.available_equipment_trend,
      trendAbs: Math.abs(s.available_equipment_trend ?? 0),
      trendLabel,
      isLive: isTodayActive,
    },
  ];
  const linkedStatCards = statCards.map((card) => {
    if (card.label === "Đang thuê") {
      return {
        ...
        onClick: () =>
          navigate(
            `/equipment?availabilityStatus=active&asOf=${dateRange.end}`,
          ),
      };
    }
    ...
    if (card.label === "Thiết bị trống") {
      return {
        ...
        onClick: () =>
          navigate(
            `/equipment?availabilityStatus=available&asOf=${dateRange.end}`,
          ),
      };
    }
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6 p-4 md:p-7 pb-10 bg-slate-50/50 min-h-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-3 relative z-10">
        <div>
          <h1 className="text-[22px] md:text-[26px] font-semibold text-slate-900 flex items-center gap-2 leading-tight tracking-tight">
            ...
            <span className="inline-block animate-[wave_1.8s_ease-in-out_infinite] origin-[70%_80%]">
              👋
            </span>
          </h1>
        ...
        {!isDriver && (
          ...
          >
            <span className="flex items-center gap-2.5">
              ...
              <span className="flex flex-col">
                <span className="text-[13px] font-bold text-slate-700">
                  {showMobileStats ? "Ẩn thống kê" : "Xem thống kê"}
                </span>
                <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">
                  {linkedStatCards.length} chỉ số tổng quan
                </span>
              </span>
            </span>
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition-colors group-hover:bg-white group-hover:text-orange-500">
              <ChevronDown
                size={12}
                className={`transition-transform duration-200 ${showMobileStats ? "rotate-180" : ""}`}
              />
            </span>
          </button>
        )}
      ...
      <div
        className={`-mt-1.5 transition-all duration-300 md:mt-0 ${statsLoading ? "opacity-40 scale-[0.99]" : "opacity-100"}`}
      >
        {!isDriver && (
          <div
            className={`relative md:mb-6 md:min-h-[120px] ${showMobileStats ? "block mb-4" : "hidden md:block"}`}
          >
            <div
              className={`${showMobileStats ? "mt-2 grid" : "hidden"} grid-cols-2 gap-3 md:mt-0 md:grid md:grid-cols-3 md:gap-4`}
            >
              {linkedStatCards.map((sc, i) => (
                <StatCard key={i} {...sc} />
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          >
            <span className="flex items-center gap-2.5">
              ...
              <span className="flex flex-col">
                <span className="text-[13px] font-bold text-slate-700">
                  {showMobileActions ? "Ẩn cần xử lý" : "Xem cần xử lý"}
                </span>
                <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">
                  {actionCount} việc tổng quan
                </span>
              </span>
            </span>
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition-colors group-hover:bg-white group-hover:text-orange-500">
              <ChevronDown
                size={12}
                className={`transition-transform duration-200 ${showMobileActions ? "rotate-180" : ""}`}
              />
            </span>
          </button>
          ...
          <div
            className={`${showMobileActions ? "grid" : "hidden"} grid-cols-1 md:grid md:grid-cols-2 xl:grid-cols-3 gap-4`}
          >
            <ActionPanel
              color="blue"
              icon={ArrowUpFromLine}
              title={isTodayActive ? "GIAO MÁY HÔM NAY" : "GIAO MÁY TRONG KỲ"}
              count={driverPickups.length}
              items={driverPickups}
              renderItem={(r) => (
                <ActionRow
                  key={r.id}
              ...
              onViewAll={() =>
                navigate(
                  `/rentals?view=pickups_today${dateRange.start && dateRange.end ? `&startDate=${dateRange.start}&endDate=${dateRange.end}` : ""}`,
                )
              }
            />
            <ActionPanel
              ...
              title={isTodayActive ? "TRẢ MÁY HÔM NAY" : "TRẢ MÁY TRONG KỲ"}
              count={driverReturns.length}
              items={driverReturns}
              renderItem={(r) => (
                <ActionRow
                  key={r.id}
            ...
            <ActionPanel
              ...
              icon={AlertTriangle}
              title="TRỄ HẠN"
              count={driverOverdue.length}
              items={driverOverdue}
              renderItem={(r) => (
                <OverdueRow
                  key={`${r._overdueType}-${r.id}`}
                  rental={r}
                  onClick={() => openEditModal(r)}
                  overdueType={r._overdueType || "return"}
                />
              )}
              onViewAll={() => navigate("/rentals?view=overdue")}
            />
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center w-full">
            {tabDefs.map((tab) => (
              ...
              >
                ...
                {tab.count > 0 && (
                  <span
                    className={`min-w-[16px] h-4 md:min-w-[20px] md:h-5 px-1 md:px-1.5 flex items-center justify-center text-[9px] md:text-[10px] font-bold rounded-full transition-colors
                ...
                <table className="w-full border-collapse text-[13px]">
                  <thead>
                    <tr className="bg-slate-50/50">
                      <SortHeader field="code" className="whitespace-nowrap">
                        Mã đơn
                      </SortHeader>
                      <SortHeader field="created" className="whitespace-nowrap">
                        Ngày tạo
                      </SortHeader>
                      <SortHeader field="customer">Khách hàng</SortHeader>
                      <SortHeader field="equipment">Thiết bị</SortHeader>
                      <SortHeader field="pickup" className="whitespace-nowrap">
                        Thời gian nhận
                      </SortHeader>
                      <SortHeader field="branch">Nơi nhận máy</SortHeader>
                      <SortHeader field="price" className="whitespace-nowrap">
                        Tổng tiền
                      </SortHeader>
                      <SortHeader field="status">Trạng thái</SortHeader>
                      <th className="px-5 py-4 text-right text-[11px] font-semibold text-slate-400 uppercase tracking-widest">
                        Thao tác
                      </th>
                    </tr>
                  </thead>
                      ...
                      return (
                        ...
                        >
                          <td className="px-5 py-4 text-[11.5px] font-semibold text-slate-400 whitespace-nowrap">
                            {r.code ||
                              `#OD${String(r.order_number || r.id).padStart(7, "0")}`}
                          </td>
                                    ...
                                    {r.items.map((it, idx) => (
                                      ...
                                      >
                                        ...
                                        <div className="min-w-0">
                                          <div className="flex items-center gap-1">
                                            <p
                                              className="font-semibold text-slate-800 text-[13px] leading-tight break-words line-clamp-2"
                                              title={it.name}
                                            >
                                              {it.name}
                                            </p>
                          ...
                          <td className="px-5 py-4 whitespace-nowrap">
                            ...
                            <p className="text-[11px] text-slate-400 mt-0.5 font-semibold uppercase tracking-tight">
                              {fmtTime(r.pickup_time)}
                            </p>
                          </td>
                            ...
                            {r.handover_user_name ? (
                              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-blue-600 bg-blue-50/80 border border-blue-100 rounded-md px-2 py-0.5 mt-1.5 w-fit">
                                ...
                                <span
                                  ...
                                  title={`Giao nhận máy: ${r.handover_user_name}`}
                                >
                          ...
                          <td className="px-5 py-4 text-[15px] font-semibold text-slate-900 whitespace-nowrap">
                            {fmtVND(r.total_price)}
                          </td>
                          <td className="px-5 py-4">
                            <span
                              className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10.5px] font-semibold whitespace-nowrap uppercase tracking-tight ${status.cls}`}
                            >
                              {status.label}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-right">
                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => openEditModal(r)}
                                className="p-2 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-orange-500 hover:border-orange-200 hover:bg-orange-50 transition-all shadow-sm cursor-pointer"
                              >
                                <Edit2 size={14} />
                              </button>
                            </div>
        ...
        {totalPages > 1 && (
          <div className="px-4 md:px-5 py-4 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-50/30">
            <p className="text-[12.5px] text-slate-500 font-semibold order-2 md:order-1">
              Hiển thị <span className="text-slate-900">{rentals.length}</span>{" "}
              / <span className="text-slate-900">{totalCount}</span> đơn
            </p>
