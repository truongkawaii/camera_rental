...
import { getPerformanceMetrics, getRevenueByBranch, getInvestorRevenue, getAdsCosts, getMiscCosts } from '../../api/client';
...
const Performance = () => {
  ...
  const loadData = async (start, end, isRefresh = false) => {
    ...
    try {
      ...
      if (isAdmin || isInvestor) {
        ...
        promises.push(getInvestorRevenue(start, end));
      }
      ...
      if (isAdmin || isInvestor) {
        ...
        setInvestorRevenue(results[4]?.data || []);
      } else {
        setAdsCosts([]);
        setMiscCosts([]);
        setInvestorRevenue([]);
  ...
  const investorGroupRows = investorRevenue
    .map(investor => ({
      ...investor,
  ...
  // Split investor orders by creation date
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <>
            ...
            {isAdmin && (
              <div className="mb-8 bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-hidden">
                <div className="p-5 border-b border-emerald-50 bg-emerald-50/30 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2 text-[11px] font-bold uppercase tracking-wider">
                    <span className="rounded-full bg-white px-3 py-1 text-slate-500 border border-emerald-100">
                      {investorOrderRows.length} đơn
                      <span className="font-bold lowercase ml-1">({totalInvestorOrdersInMonth} mới</span>
                      {totalInvestorOrdersBeforeMonth > 0 && (
                        <span className="font-bold lowercase"> + {totalInvestorOrdersBeforeMonth} cũ</span>
                      )}
                      {totalInvestorCancelledOrders > 0 && (
                        <span className="font-bold lowercase text-rose-400"> + {totalInvestorCancelledOrders} hủy</span>
                      )}
                    ...
                    <span className="rounded-full bg-white px-3 py-1 text-orange-500 border border-orange-100">
                            ...
                            <div className="lg:hidden p-3 space-y-2.5 max-h-[700px] overflow-y-auto custom-scrollbar">
                              {investor.orders.map(order => {
                                ...
                                <div key={`${investor.id}-${order.id}`} className="rounded-2xl border border-slate-100 bg-white overflow-hidden shadow-sm">
                                  {/* Header row with code + status + revenue */}
